...
- コードやドキュメントを修正する前に、リモートの最新状態を取得し、現在のブランチがリモートで削除済みではないか確認してください。
- 現在のブランチがリモートで削除済み、または別件の作業ブランチである場合は、`main` の最新状態を元に新しい feature/fix/docs ブランチを作成してから作業を開始してください。
